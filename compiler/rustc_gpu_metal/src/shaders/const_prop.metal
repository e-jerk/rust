...
kernel void const_prop(
    const device uint* configs [[buffer(0)]],
    const device uint* const_facts [[buffer(1)]],
    ...
    for (uint stmt_idx = 0; stmt_idx < stmt_count; stmt_idx++) {
        uint fact = const_facts[block_idx * pc.facts_stride + stmt_idx];
        if (fact != 0xFFFFFFFF) {
